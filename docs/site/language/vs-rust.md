...
Milo's decided default is to trap overflow in every build mode. Today the trap is gated to `--debug` — `run`, `build`, and `--release` wrap silently, Rust-parity but not the target — while div-by-zero and `INT_MIN / -1` already trap everywhere.
:::
